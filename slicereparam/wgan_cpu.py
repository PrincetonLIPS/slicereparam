...
for epoch in range(num_epochs):
    for i in range(num_iters):
        ...
        # currently ignore burn in during discriminator phase
        # us, norm_ds, x0b, key = generate_randomness_burnin(key)
        # xs, _, _, _ = forwards(burn_in, theta, x0b, us, norm_ds)
        # x0 = xs[-1]
        ...
        dL_dxs = dL_dxs.reshape((num_chains, Sc+burn_in, D))
        ...
        #     dL_dtheta = dL_dtheta + backwards3(Sc, theta, us[:,j,:], norm_ds[:,j,:], 
        #                                         xs_j, xLs[:,j,:], xRs[:,j,:], alphas[:,j,:], dL_dxs[j*Sc:(j+1)*Sc], ys)

        xs0, us, norm_ds, xLs, xRs, alphas = swap_axes(xs0, us, norm_ds, xLs, xRs, alphas)
        dL_dthetas = vmapped_backwards(Sc+burn_in, theta, us, norm_ds, xs0, xLs, xRs, alphas, dL_dxs, ys)
        dL_dtheta = np.sum(dL_dthetas,axis=0)
        # print("Diff: ", np.linalg.norm(dL_dtheta - dL_dtheta2))
        # np.allclose(dL_dtheta, dL_dtheta2)
        # t2 = time.time()
        # print((t2-t1))
        ...
        losses.append(total_loss(xs[1:], phi))
...
# np.savez("wgan_circle.npz", theta=theta, phi=phi, losses=np.array(losses), \
    # m=m, v=v, adam_iter=adam_iter, m_phi=m_phi, v_phi=v_phi, adam_iter_phi=adam_iter_phi)
# pbar.close()
...
xmax =2.0
plt.figure()
visualize_2D(log_pdf, theta, xmin=xmin, xmax=xmax, vmin=0.0, vmax=0.5, dx=0.1, log=True)
plt.title("Iteration " + str(adam_iter))
...
xmax =3.0
plt.figure()
visualize_2D(discriminator, phi, xmin=xmin, xmax=xmax, vmin=0.0, vmax=0.25, dx=0.1, log=True)
plt.title("Iteration " + str(adam_iter))
plt.plot(X[:500,0], X[:500,1], 'r.', markersize=2.5)
