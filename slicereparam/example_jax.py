...
def forwards(S, theta, x, f_alpha, us, ds, jit_brentq_l, jit_brentq_r):
    xs = [x]
    xLs = []
    xRs = []
    alphas = []
...
xstar = random.normal(subkey, (D,))
loss_fun = lambda x : np.mean((x - xstar)**2)

# gradient of loss with respect to x
dL_dx = jit(grad(loss_fun))

# run forward pass
xs, xLs, xRs, alphas = forwards(S, theta, x, f_alpha, us, ds, jit_brentq_l, jit_brentq_r)
...
for m, v in enumerate(np.eye(M)):
    ...
    theta1 = theta - dx * v
    theta2 = theta + dx * v
    xs1, xLs1, xRs1, alphas1 = forwards(S, theta1, x, f_alpha, us, ds, jit_brentq_l, jit_brentq_r)
    xs2, xLs2, xRs2, alphas2 = forwards(S, theta2, x, f_alpha, us, ds, jit_brentq_l, jit_brentq_r)
